...
```{r maPlotsTumor, fig.height=20, fig.width=6, dpi=100, echo=FALSE, fig.cap="Figure S4: MA-plots of the tumor samples."}
...
```{r maPlotsNormal, fig.height=20, fig.width=6, dpi=100, echo=FALSE, fig.cap="Figure S5: MA-plots of the normal samples."}
par(mfrow=c(6,3), mar=c(4, 5, 3, 1))
...
```{r sampleClustering_2, fig.height=7, fig.width=14, dpi=100, echo=TRUE, fig.cap="Figure S8: Hierarchical clustering of the samples."}
par(mfrow=c(1, 1))
batch <- as.integer(factor(tss))
sampleDendrogram <- as.dendrogram(sampleClustering, hang=0.1)
names(batch) <- colnames(se)
...
```{r mds3, fig.height=7, fig.width=14, dpi=100, echo=TRUE, fig.cap="Figure S9: Multidimensional scaling plot of the samples."}
plotMDS(dge, labels=outcome, col=batch)
legend("bottomright", paste("Batch", sort(unique(batch)), levels(factor(tss))),
       fill=sort(unique(batch)))
```
...
```{r sampleClustering_3, fig.height=7, fig.width=14, dpi=100, echo=TRUE, fig.cap="Figure S10: Hierarchical clustering of the samples."}
par(mfrow=c(1, 1))
batch <- as.integer(factor(tss))
sampleDendrogram <- as.dendrogram(sampleClustering, hang=0.1)
names(batch) <- colnames(se)
...
There are `r sum(p.adjust(pv, method="fdr") < 0.01)` genes changing significantly their expression at FDR < 1%. In Figure S11 below we show the distribution of the resulting p-values.

```{r pdist, echo=FALSE, out.width="500px", fig.cap="Figure S11: Distribution of raw p-values for an F-test on every gene between tumor and normal samples."}
hist(pv, main="", las=1)
```
...
```{r psvdist, echo=FALSE, out.width="500px", fig.cap="Figure S12: Distribution of raw p-values for an F-test on every gene between tumor and normal samples, adjusting for surrogate variables estimated with SVA."}
...
Using this simple model, the results obtained are **`r length(DEgenes)` DE genes**. As it can be seen in the chromosome distribution, the chromosome with more DE genes is `r names(sort(table(tt$chr[tt$adj.P.Val < FDRcutoff]), decreasing = TRUE)[1])`.


Two useful diagnostic plots (Figure S13) for DE analysis are the distributions of p-values and moderated t-statistics:

```{r fit1_diagnostic, fig.height=4, fig.width=10, out.height="300px", echo=FALSE, fig.cap="Figure S13: Distribution of p-values and moderated t-statistics for the directly fitted model"}
...
```{r fit2_voom, fig.height=6, fig.width=7, out.height="500px", tidy=FALSE, fig.cap="Figure S14: Mean-variance relationship at individual observation-by-gene level"}
...
```{r fit2_diagnostic, fig.height=4, fig.width=10, out.height="300px", echo=FALSE, fig.cap="Figure S15: Distribution of p-values and moderated t-statistics for the model with voom weights"}
par(mfrow = c(1, 2), mar = c(4, 5, 2, 2))
...
There are **`r sv$n` SVs**. Second, we add these SVs to the design matrix, which has the following form:
...
Using this approach, there are **`r length(DEgenes4)` DE genes**. As it can be seen in the chromosome distribution, the chromosome with more DE genes is `r names(sort(table(tt4$chr[tt4$adj.P.Val < FDRcutoff]), decreasing = TRUE)[1])`.


Now, it is possible to examine the diagnostic plots (Figure S16) for the DE analysis using this approach:

```{r fit4_diagnostic, fig.height=4, fig.width=10, out.height="300px", echo=FALSE, fig.cap="Figure S16: Distribution of p-values and moderated t-statistics for the model with known covariates and SVA"}
par(mfrow = c(1, 2), mar = c(4, 5, 2, 2))
...
--------------|----------|------------------------
1. Fit directly the linear model  | `r length(DEgenes)` | `r names(sort(table(tt$chr[tt$adj.P.Val < FDRcutoff]), decreasing = TRUE)[1])`
2. Adjust for mean-variance relationship | `r length(DEgenes2)` | `r names(sort(table(tt2$chr[tt2$adj.P.Val < FDRcutoff]), decreasing = TRUE)[1])`
3. Adjust for known covariates | `r length(DEgenes3)` | `r names(sort(table(tt3$chr[tt3$adj.P.Val < FDRcutoff]), decreasing = TRUE)[1])`
4. Adjust for known covariates + SVA | `r length(DEgenes4)` | `r names(sort(table(tt4$chr[tt4$adj.P.Val < FDRcutoff]), decreasing = TRUE)[1])`
...
```{r volcanoPlot, echo=FALSE, fig.height=6, fig.width=10, out.height="500px", fig.cap="Figure S17: Volcano plots for models 2 and 4"}
...
```{r maPlot, echo=FALSE, fig.height=6, fig.width=10, out.height="500px", fig.cap="Figure S18: MA-plot for model 4"}
par(mfrow=c(1, 1))
...
```{r vennDiagram, echo=FALSE, fig.height=4, fig.width=5, out.height="300px", fig.cap="Figure S19: VennDiagram: Overlap of DE genes between contrasts of interest"}
